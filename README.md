...
MVP2 在 MVP1 的 RAG 闭环基础上，补齐了文档管理、对话历史和多轮会话等真实使用体验，并建立了 SDD 基础骨架。当前 MVP3 之前的核心阶段已实现；需要注意，重新索引依赖 MinIO 原文件存储，默认配置下 MinIO 未启用。
...
MVP3 将优先完善基础设施，规划 SDD 驱动开发、LangGraph 意图识别与路由、混合检索、RRF 融合排序、可选 rerank、MySQL 兼容、异步索引、监控指标和 SSE 流式输出。API Key/JWT、多租户、权限过滤和管理员配置后移到 MVP4。
